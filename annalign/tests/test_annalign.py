...
def test_downsample():
    ...
    for tr in trs:
        ...
        if np.allclose(down_array, in_array):
            print("Passed for temporal resolution of {}".format(tr))
    ...
    if np.allclose(tr_3_vals, out_0_3_array):
        print("tr 0.3 checks out")

    tr = 0.5
    tr_5_vals = [0, 0, 0.5, 0.5, 1.0, 1.5, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]
    out_0_5_array = r_over_list(vals, tr)
    if np.allclose(tr_5_vals, out_0_5_array):
        print("tr 0.5 checks out")

    tr = 1.0
    tr_1_vals = [0, 0, 0, 0, 0, 1.0, 1.0, 2.0, 2.0, 2.0, 3.0, 4.0, 4.0]
    out_1_array = r_over_list(vals, tr)
    if np.allclose(tr_1_vals, out_1_array):
        print("tr 1.0 checks out")

    # just to be safe super high trs
    tr = 10.0
    tr_10_vals = np.zeros(len(vals))
    out_10_array = r_over_list(vals, tr)
    if np.allclose(tr_10_vals, out_10_array):
        print("tr 10 checks out")
